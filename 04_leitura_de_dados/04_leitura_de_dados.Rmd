...
3. Importando dados a usando o pacote `readr`;
4. Importando dados a usando o pacote `readxl`;
5. Exportando dados.
...
* __Dica do R:__ você sabe a diferença entre um `error`, `warning` e uma `message`?
    + `error`: ocorreu um erro durante o processamento e o que quer que você tenha pedido para ser feito não aconteceu;
    + `warning`: o processamento foi feito, mas alguma coisa não correu como o esperado e pode ter comprometido o resultado do processamento;
    + `message`: o processamento foi feito, tudo correu bem e só uma observação, que não compremete a tarefa, é apresentada.
