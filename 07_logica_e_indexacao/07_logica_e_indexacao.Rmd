...
* Além dos operadores lógicos, o R possui um conjunto de funções que também podem ser empregadas para testes lógicos: `is.na`, `is.infinite`, `is.nan`, `setequal`, `setdiff`, `which`, `which.min`, `which.max` e `ifelse`.
...
* No entanto, as vezes queremos saber qual a posição absoluta em que um determinado elemento que satisfaça uma condição está, para isto existem as funções: `setequal`, `setdiff`, `which`, `which.min`, `which.max` e `ifelse`.
