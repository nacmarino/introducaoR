...
a. Passe todos os nomes das colunas para caixa baixa;  
b. Faça com que os nomes das colunas fiquem mais amigáveis a serem usados no R.  
...
a. Sem alterar os nomes das colunas de `dados`, faça com que as colunas de 1 a 9 passem a se chamar P1 a P9.  
...
a. Crie uma coluna chamada `usuario`, que deve conter se este é o primeiro curso ou não que a pessoa faz sobre o R;  
...
c. Com o resultado da última opera, ordene as linhas de acordo com a ordem decrescente dos anos;
d. Além de ordenar as linhas pela ordem decrescente dos anos, ordem agora também o tamanho da população.  
...
```{r}
mutate(gapminder, log_gdp = log(gdpPercap))
```
...
b. Qual a expectativa de vida máxima dos países Europeus?
c. Qual a expectativa de vida mínima dos países Asiáticos e Africanos?
...
b. Quantas pessoas são mestrandas e quantas são doutorandas? (`pergunta.1`)
c. Quanto tempo falta, em média para a defesa dos mestrandos e dos doutorandos? (`pergunta.1` e `pergunta.8`)
...
dados <- dados %>% 
  mutate(pergunta.7 = ifelse(str_detect(string = pergunta.7, pattern = "Recomendação de outros professores") == TRUE, 
                             "Professores", ifelse(str_detect(string = pergunta.7, pattern = "Circulação Interna Grade do PPGE/UFRJ") == TRUE,
                                                   "Grade Interna", ifelse(str_detect(string = pergunta.7, pattern = "Recomendação do Orientador") == TRUE, "Orientador", pergunta.7))))
```
...
```{r echo=FALSE}
dados <- dados %>% 
  select(identidade, turma, usuario, programa, instituicao, pergunta.1, pergunta.2, pergunta.4, pergunta.5, pergunta.7, pergunta.8, pergunta.9) %>% 
  rename(ID = identidade, Cursando = pergunta.1, Formacao = pergunta.2, "Tempo de uso" = pergunta.4, "Qts cursos feitos" = pergunta.5, 
         "Quem recomendou" = pergunta.7, "Prazo defesa" = pergunta.8, "Conhecimento estatistica" = pergunta.9) %>% 
  mutate("Conhecimento estatistica"  = sub(pattern = "á", replacement = "a", x = "Conhecimento estatistica"),
         "Conhecimento estatistica"  = sub(pattern = "é", replacement = "e", x = "Conhecimento estatistica"),
         "Conhecimento estatistica"  = sub(pattern = "í", replacement = "i", x = "Conhecimento estatistica"))
```
...
```{r}
write.table(x = dados, file = "dados/dados_limpos.xls", sep = "\t")
```
