...
par(mar=c(1,1,1,1))
map("world","Brazil")
map.axes() # para colocar os eixos do gráfico já com longitude e latitude
```
...
map("world","Brazil")
map.axes()
map.scale(ratio = F, cex = 0.7) # para colocar a escala do mapa
abline(h = 0, lty = 2)
```
...
map(add=T) # para plotar os limites geográficos dos países sob o gráfico existente
map.axes()
map.scale(ratio=F, cex=0.7)
abline(h=0, lty = 2) # para colocar o equador
...
map.scale(ratio=F, cex=0.7)
abline(h=0, lty = 2)
map.grid(m, col="grey50", font=1, cex=0.7, pretty = T, labels = FALSE) #library(mapproj)
```
...
br <- maptools::readShapePoly("shapefiles/brasil/BRA_adm_shp/BRA_adm0") # 0: país, 1: estados, 2: municípios
estados1 <- readShapePoly("shapefiles/brasil/BRA_adm_shp/BRA_adm1") # 0: país, 1: estados, 2: municípios
...
brasil.mapa <- borders("world", regions = "Brazil", fill = "grey90", colour = "black")
...
library(raster)
alt <- raster("Dia 5/Rmaps/rasters/h_dem/hdr.adf")
plot(alt)
```
...
arquivos <- list.files("Dia 5/Rmaps//rasters", full.names = T)
head(arquivos)
env <- stack(arquivos)
```
