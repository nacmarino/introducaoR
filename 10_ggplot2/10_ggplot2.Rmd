...
* Você pode carregar individualmente o pacote `ggplot2` ou através do pacote `tidyverse`.
...
* Vamos carregar um conjunto de dados para trabalharmos: aquele das Palestras do PPGE.

```{r}
# para carregar os dados
dados <- read_delim("../04_leitura_de_dados/dados/lista_de_presenca.csv", delim = ";")
# dando uma olhada nos dados
dados
```

## Função básica: _qplot_.

* É similar à função `plot` do pacote `base`, mas possui os elementos do _The Grammar of Graphics_.  
* O objetivo é que ele seja um _quick plot_ (por isso, `qplot`).
...
* Um gráfico rápido: número total de pessoas em cada uma das categorias de formação (pessoas estão sendo contadas múltiplas vezes).

```{r fig.height=3, fig.width=4}
qplot(x = formacao_ouvinte, data = dados)
```

* Podemos adicionar elementos gráficos, baseado em alguma informação sobre os dados.  

```{r fig.height=3, fig.width=4}
qplot(x = formacao_ouvinte, data = dados, fill = origem_ouvinte)
```
...
##  Outras funcionalidades disponíveis no ggplot2
...
## Adicionar algum texto ao gráfico
...
* O site/Livro _"R for Data Science"_;
* O Google.  