...
* Uma forma de fazer isso é através de funções disponíveis na linguagem R.
...
* Também podemos pedir ao R para que repita uma sequência de valores.
...
* Podemos criar uma vetor seguindo esta indexação.
...
* Cada objeto criado pode ser descrito por um conjunto de dimensões (copie e rode os exemplos junto de cada caso abaixo):
    1. `vector`: possui apenas uma dimensão (o comprimento linear do vetor).
        + `vetor1`
    2. `matrix`: possui duas dimensões (as linhas e as colunas);
        + `matrix(c(vetor1, vetor2), ncol = 2)`
        + `array` é um caso de matriz com mais de duas dimensões: e.g. `array(0, dim = c(2, 4, 3))`
    3. `list`: possui uma ou mais dimensões (sub-listas, compostas por uma ou sub-listas e as dimensões dos objetos que as compõem);
        + `list(vetor1, vetor2, list(vetor1, vetor2), list(list(vetor1, vetor2), vetor1))`
    4. `data.frame`: possui apenas duas dimensões (as linhas e as colunas)
...
* Note que, em cada um destes casos, cada elemento pode ser rastreado até uma posição específica. Por exemplo:
    + no `vetor1`, o número 7 ocupa a sétima posição na sequência;
...
* Para tal, utilizaremos três tipos de operadores:
    1. `[]`  extrai um ou mais elementos em vetores, listas, ou data frames;
    2. `[[]]` extrai um ou mais elementos em listas ou data frames;
    3. `$` extrai elementos pelo __nome__ em listas e data frames.
...
* A indexação está usando um vetor de um, ou mais elementos, que marca a posição dos elementos a serem retidos.  
...
* Para excluirmos uma sequência contínua de valores devemos concatenar estes valores primeiro.
...
* Lembre-se que um `data.frame` possui duas dimensões: linhas e colunas.
    ...
    + Podemos ver um `data.frame` como um jogo de batalha naval: [linha, coluna]
